...
详情请戳→[简述forEach()、map()、every()、some()和filter()的用法](http://fang-kang.gitee.io/views/JS/array.html)

## 重点总结：

reduce() 是数组的**归并方法**，与forEach()、map()、filter()等**迭代方法**一样都会对数组每一项进行遍历，但是reduce() 可同时将前面数组项遍历产生的结果与当前遍历项进行运算，这一点是其他迭代方法无法企及的
